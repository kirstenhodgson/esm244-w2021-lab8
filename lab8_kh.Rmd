---
title: 'Lab 8: Clustering (k-means & hierarchical)'
...
In the week 8 lecture, we learned that for k-means clustering you need to specify the number of clusters *a priori*. R **does** have some tools to help you decide, but this should NOT override your judgment based on conceptual or expert understanding.

Here, we use the `NbClust::NbClust()` function, which "provides 30 indices for determining the number of clusters and proposes to user the best clustering scheme from the different results obtained by varying all combinations of number of cluster, distance measures, and clustering methods". See `?NbClust` for more information.
...
We're still going to use 3 clusters and see how it does, though there may be a case here for 2 given that Adelie & Chinstrap penguins are pretty similar. We're going to do this with *complete cases* - in other words, for the variables we're using to perform k-means clustering on penguins (bill length, bill depth, flipper length, body mass), we are *dropping any observation (row) where any of those are missing*. Keep in mind that this may not be the best option for every scenario - in other cases (e.g. when we have a large proportion of missingness), we may want to impute missing values instead.
...
#Now plot bill dimensions and map species & cluster number to the point shape and color aesthetics:
ggplot(penguins_cl) +
...
Takeaway: as we see from the graph, *most* chinstraps are in Cluster 1, and *most* Adelies are in Cluster 2, and *all* Gentoos are in Cluster 3 by k-means clustering. So this actually does a somewhat decent job of splitting up the three species into different clusters, with some overlap in Cluster 1 between Adelies & chinstraps, which is consistent with what we observed in exploratory data visualization.

That's your intro to k-means clustering with penguins!

#### Part 2. Cluster analysis: hierarchical
In this section, we'll be performing hierarchical cluster analysis (& making dendrograms) in R.

We will use the `stats::hclust()` function for agglomerative clustering, using WorldBank environmental data (simplified), wb_env.csv.
...
Here, let's use complete linkage (recall from lecture that clusters are merged by the smaller *maximum* distance between two observations in distinct clusters).
...
Let's update the linkage to single linkage (recall from lecture that clusters are merged by the *smallest* distance between observations in separate clusters):
...
First, we'll convert to class `dendrogram`, them combine them into a list:

```{r}
#Convert to class dendrogram
dend_complete <- as.dendrogram(hc_complete)hc_single
